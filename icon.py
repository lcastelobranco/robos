...
def parser(data, linha):
    str_sql = "INSERT INTO icon (data,ticker,nome,tipo,quantidadeteorica,participacao) values (%s,%s,%s,%s,%s,%s)"
    cursor.execute(str_sql, tuple([data] + linha.split('\n\n')[1:-1]))
    conn.commit()


conn ,cursor,engine = conection("robos_b3")
lista = contexto(cursor,"icon")


url = "http://bvmf.bmfbovespa.com.br/indices/ResumoCarteiraTeorica.aspx?Indice=ICON&idioma=pt-br"
...
try:
    ...
    error('ICON - algum erro do processo em geral')