...
def parser(data, linha):
    str_sql = "INSERT INTO ifnc (data,ticker,nome,tipo,quantidadeteorica,participacao) values (%s,%s,%s,%s,%s,%s)"
    cursor.execute(str_sql, tuple([data] + linha.split('\n\n')[1:-1]))
    conn.commit()


conn ,cursor,engine = conection("robos_b3")
lista = contexto(cursor,"ifnc")
...
try:
    ...
    if data not in lista:
        ...
        with open(r"checklist.txt", 'a') as checklist:
            checklist.writelines('IFNC\n')


except:
    error('IFNC - algum erro do processo em geral')